...
#define CREATE_VARIABLE_NODE(NAME,ARGC,...) \
do{\
    Variable* variable=VariableCreate(@$.first_line,@$.first_column,NAME);\
    AstNode* ast_node=AstNodeCreate(false,variable);\
    $$=KTreeCreateNodeWithChidren(&ast_node,ARGC,#__VA_ARGS__);\
}while(false)

#define CREATE_EMPTY_VARIABLE_NODE() \
do{\
    $$=NULL;\
}while(false)
